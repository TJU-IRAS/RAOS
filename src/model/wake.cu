...
<<<<<<< HEAD
__device__ float3 biot_savart_induction(VortexMarker_t a, 
    VortexMarker_t b, 
    float3 p,
	float3 vel
)
...
__device__ float3 tile_calculation_vel_markers(float3 pos,
	float3 vel,
	int tile,
	int tile_size,
...
)
{
    ...
    */
    ...
    // strength ---- part of Equ (2-8) of Luo Thesis
    scale = a.Gamma * db_h * 10000.0f * __frsqrt_rn(__powf(a.r * 100, 4) + db_h * db_h * 10000.0f * 10000.0f) *
            (cos_apab + cos_babp) / (4 * PI);
     ...
     int num_markers)
{
    ...
    for (i = 0; i < tile_size - 1; i++)
    {
        ...
        else
        {
            ...
            // if this marker is not an end point
            if (isend == false)
            {
                vel = biot_savart_induction(tile_markers[shared_mem_offset + i],
                                            tile_markers[shared_mem_offset + i + 1], pos, vel);
            }
        }
    }

    return vel;
>>>>>>> e6d306e260c875ff6132332eee4659d5d1cc6b5b
}
 ...
 */
<<<<<<< HEAD
__global__ void CalculateVelofMarkers(VortexMarker_t *markers, 
    int *idx_end, 
    int num_fila,
	int num_markers,
	int tile_size,
	int row_sgmts, 
	Wake_FreeStreamVector_t	*wind,
	int *idx_end_robot,
	int num_robot
)
{
...
)
{
	...
	if (tid < num_markers)
		...
		mkr;
=======
__global__ void CalculateVelofMarkers(
    VortexMarker_t *markers,
    int *idx_end,
    int num_fila,
    int num_markers,
...
)
{
    ...
    while (true) // every thread can enter this loop
    {
        ...
        __syncthreads(); // make sure the shared mem has been loaded
        if (row < num_markers)
            vel = tile_calculation_vel_markers(pos, vel, idx / tile_size, tile_size, row_sgmts, markers, idx_end,
                                               num_fila,
                                               num_markers);
...
/* calculate vortex core radius of markers */
__global__ void CalculateVtxCoreofMarkers(
    VortexMarker_t *markers,
    int num_markers,
    int *idx_end,
    int num_fila,
...
)
{
    ...
    if (tid < num_markers)
    {
        ...
        for (int i = 0; i < num_fila; i++)
        {
            if (idx_end[i] < tid)
            {
                // find which filament this marker belongs to
                if (idx_end[i] > fila_start)
                    fila_start = idx_end[i];
            }
            else
...
__device__ void induced_velocity_vortex_ring(
    float *center,
    float radius,
    float Gamma,
    float core_radius,
...
void WakesUpdate(
    ...
    const char *integration_scheme,
    SimState_t *sim_state,
    SimEnvInfo *sim_env_info)
>>>>>>> e6d306e260c875ff6132332eee4659d5d1cc6b5b
{
...
=======
FilaState_t *virtual_plume_puffs;     // on-host ...
FilaState_t *dev_virtual_plume_puffs; // on-device array containing the states of virtual plume puffs

__global__ void CalculateIndVelatPlumePuffs(
    VortexMarker_t *markers,
    int *idx_end,
    int num_fila,
    int num_markers,
...
void WakesInvVelatAdjacentCubeInit()
{
    ...
    //printf("=== max_virtual_puff_num %d ===", max_virtual_puff_num);

    // allocate a page-locked host memory containing all of the plume puffs states
    HANDLE_ERROR(cudaHostAlloc((void **) &virtual_plume_puffs,
                               max_virtual_puff_num * sizeof(*virtual_plume_puffs), cudaHostAllocDefault));
    // allocate device memory as big as the host's
    HANDLE_ERROR(cudaMalloc((void **) &dev_virtual_plume_puffs,
                            max_virtual_puff_num * sizeof(*dev_virtual_plume_puffs)));
}

void WakesInvVelatAdjacentCubeUpdate(std::vector<Robot*> *robots, std::vector<FilaState_t> *plume_output)
{
    unsigned int idx_robot, idx_rotor;
    int idx_blade;
    int addr_cp_markers = 0, num_blade = 0;
    ...
    //  Todo: generate virtual puffs in the adjacent cube around the robot
    std::vector<FilaState_t> plume_vec;

    assert(robots->size() == 1);
    float origin_pos[3] = {0};
    origin_pos[0] = robots->at(0)->state.pos[0];
    origin_pos[1] = robots->at(0)->state.pos[1];
    origin_pos[2] = robots->at(0)->state.pos[2];

    float cube_length = 2.0f;
    float puff_length = 0.1f;

    for ( float x = origin_pos[0] - cube_length / 2; x < origin_pos[0] + cube_length / 2; x += puff_length )
    {
        for (float y = origin_pos[1] - cube_length / 2; y < origin_pos[1] + cube_length / 2; y += puff_length)
        {
            for (float z = origin_pos[2] - cube_length / 2; z < origin_pos[2] + cube_length / 2; z += puff_length)
            {
                FilaState_t plume;
                plume.pos[0] = x;
                plume.pos[1] = y;
                ...
                plume_vec.emplace_back(plume);
            }
        }
    }

    //printf(" === plume_vec len %d === \n", static_cast<int>(plume_vec.size()));

    std::copy(plume_vec.begin(), plume_vec.end(), &virtual_plume_puffs[0]);
    ...
    HANDLE_ERROR(cudaMemcpy(dev_idx_end_marker_fila, idx_end_marker_fila,
                            num_blade * sizeof(int), cudaMemcpyHostToDevice));
    HANDLE_ERROR(cudaMemcpy(dev_virtual_plume_puffs, virtual_plume_puffs,
                            plume_vec.size() * sizeof(FilaState_t), cudaMemcpyHostToDevice));
    ...
    cudaError_t err;
    //  <1> calculate velocity of Lagrangian markers, n-1 state
    blocks = (plume_vec.size() + p - 1) / p;
    //  Note: here omitted checks for max number of blocks, since in RAO problem the vortex markers
    //        rarely exceeds 65535*threads.
    CalculateIndVelatPlumePuffs << < blocks, threads, threads * sizeof(VortexMarker_t) >> >
                                                      (dev_wake_markers, dev_idx_end_marker_fila, num_blade, addr_cp_markers, p, q, dev_virtual_plume_puffs, plume_vec.size());
    err = cudaGetLastError();
    if (err != cudaSuccess)
        printf("Error: %s\n", cudaGetErrorString(err));

    // Step 4: distribute puffs to host and give back
    HANDLE_ERROR(cudaMemcpy(virtual_plume_puffs, dev_virtual_plume_puffs,
                            plume_vec.size() * sizeof(FilaState_t), cudaMemcpyDeviceToHost));
    plume_output->clear();
    //std::copy(&virtual_plume_puffs[0], &virtual_plume_puffs[plume_vec.size()], plume_output->data());
    std::copy(&virtual_plume_puffs[0], &virtual_plume_puffs[plume_vec.size()], plume_vec.data());
    plume_output->insert(plume_output->begin(), plume_vec.begin(), plume_vec.end());
}
